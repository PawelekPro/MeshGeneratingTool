...
{
  return STANDARD_TYPE(ShapeIdAttribute);
}
...
Standard_Boolean XmlShapeLibDrivers_ShapeIdDriver::Paste(
    ...
    Handle(ShapeIdAttribute) shapeIdAttr = Handle(ShapeIdAttribute)::DownCast(theTarget);
...
void XmlShapeLibDrivers_ShapeIdDriver::Paste(
    ...
    Handle(ShapeIdAttribute) attr = Handle(ShapeIdAttribute)::DownCast(theSource);
    if (attr.IsNull()) {
        return;
    }
