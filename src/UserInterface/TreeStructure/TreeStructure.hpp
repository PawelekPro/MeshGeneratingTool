...
public:
	...
	/**
	 * @brief Creates a sub-item under specified parent TreeItem based on the passed ItemType. 
	 * 
	 ...
	 * 
	 * @param aFilePath - file path string that will be displayed in corresponding widget.
	 * and stored in created QDomElement associated with new ImportSTEP item.
	 * @return Pointer to the newly added TreeItem representing the ImportSTEP item.
	 */
	 ...
	 * 
	 * @param aShapesTags - tags of shapes that will be affected by element sizing.
	 * @param aSelectionType - type of shapes that will be affected by element siizng. 
	 * @return Pointer to the newly added TreeItem representing the ImportSTEP item.
	 */
	TreeItem* addElementSizingItem(const std::vector<int>& aShapesTags, const IVtk_SelectionMode& aSelectionType);

	/**
	 * @brief Removes the TreeItem from TreeWidget without deleting it. This takes away ownership,
	 * from TreeStructure - possible memory leak if the item will not be explicitly deleted.
	 * 
	...
	/**
	 * @brief Adds existing TreeItem to the TreeStructure under given parent item. 
	 * 
	...
	/**
	 * @brief returns the root TreeItem based on the spcified ItemTypes::Root enum.
	 * 
	...
	/**
	 * @brief Returns a QList collection of all sub TreeItems with specified ItemTypes::Sub type. 
	 * 
	 * @param aSubType - ItemTypes::Sub enum, 
	 * @return QList with all TreeItem* of type aSubType that are currently children to TreeStructure.
	 */
