...
private:
	MeshRenderHandler(
		QVTKRenderWindow* aRenderWindow, const ModelDataView& aModelDataView, QObject* aParent);

public slots:

	/**
	 * @brief Slot that upon triggered will clear the renderer, fetch current MeshActor from
	 * model view and add it to the renderer.
	 */
	void showMeshActor();
