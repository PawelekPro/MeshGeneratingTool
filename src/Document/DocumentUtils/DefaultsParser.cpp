	...
	try {
		...
		if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
			throw std::runtime_error(
				"Failed to open JSON file: " + aJsonFilePath.toStdString());
		}
		...
		if (doc.Parse(fileContent.toStdString().c_str()).HasParseError()) {
			throw std::runtime_error("Failed to parse JSON content from file: "
				+ aJsonFilePath.toStdString());
		}
...
QDomElement getItemProperties(
	...
	rapidjson::Document aJsonDoc
		= initJsonDocument(AppInfo::rootItemsSetupPath);
	QString itemTypeLabel = ItemTypes::label(aItemType);
	QDomElement properties
		= getPropertiesFromJson(aDomDoc, aJsonDoc, itemTypeLabel);
	return properties;
}
...
QDomElement getItemProperties(
	...
	QDomDocument tempDocument = dom_document; // temporary copy
	rapidjson::Document aJsonDoc
		= initJsonDocument(AppInfo::rootItemsSetupPath);
	const QString itemTypeLabel = ItemTypes::label(aItemType);

	return getPropertiesFromJson(tempDocument, aJsonDoc, itemTypeLabel);
}
...
QStringList getComboBoxListFromJson(
	...
	if (!aJsonDoc.HasMember(comboBoxLabel)
		...
		qWarning() << "Json document does not include entry: "
				   << aComboBoxEntry;
		return QStringList();
	}
	...
	for (const auto& pair : jsonValue.GetObject()) {
		...
		const QString label
			= QString::fromStdString(pair.value["label"].GetString());
...
QDomElement getPropertiesFromJson(QDomDocument& aDomDoc,
	const rapidjson::Document& aJsonDoc, const QString& aJsonItemEntry) {
	...
	if (!entrySection.HasMember("Properties")
		|| !entrySection["Properties"].IsArray()) {
	...
	for (auto itr = propertiesArray.Begin(); itr != propertiesArray.End();
		++itr) {
		if (!itr->IsObject()) {
...
QDomElement jsonValueToProperty(
	QDomDocument& aDomDoc, const rapidjson::Value& aJsonValue) {
	QDomElement property = aDomDoc.createElement("Property");

	for (auto itr = aJsonValue.MemberBegin(); itr != aJsonValue.MemberEnd();
		++itr) {
		...
		if (key == "value") {
			...
			Properties::setPropertyAttribute(
				property, key, valueVariant.toString());
		}
	}
